  ...
  return (page: DvPage): DvPage[] => {
    ...
    switch (pageType) {
      ...
      case "kinded > subcategory":
        ...
        const catDefnTag = getCategories(p)(page)?.pop()?.defnTag;
        const catPage = (
          (pageType === "kinded" || pageType === "kinded > subcategory")
          && catDefnTag
        )
          ? getPagesFromTag(p)(
            catDefnTag,
          ...
          : page.file.tags.some(i => i.startsWith(`#type/`))
            ...
            : kindPage && isPageReference(kindPage.type)
              ...
              : pageType === "kinded > category" && hasTypeTag(p)(page)
                ...
                : pageType === "kinded" && catPage && isPageReference(catPage.type)
                  ? [getPage(p)(catPage.type)]
                  : [] as DvPage[];
      case "multi-kinded":
      ...
      case "multi-kinded > subcategory":
        ...
        return isArray(page.types) && page.types.some(i => isPageReference(i))
          ...
          : page.file.tags.some(i => i.startsWith(`#type/`))
            ? (Array.from(page.file.tags) as string[]).filter(i => i.startsWith("#type/")).map(i =>
