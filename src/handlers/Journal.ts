...
import {  retainUntil, WHITESPACE_CHARS } from "inferred-types";
...
export const Journal = createHandler("Journal")
	...
	.handler( async(evt) => {
		...
		if (page) {
			let when = getWhenDate(p)(page);
			
			if (when) {
				const format = evt.options.fileFormat || "journal/YYYY/YYYY-MM-DD";
				/** previous day link */
				const prev = htmlLink(p)(
					journalFile(format, priorDay(when)),
					{ 
				...
				/** next day link */
				const next = htmlLink(p)(
					journalFile(format, nextDay(when)),
					{  
						createPageWhereMissing: true, 
						display: describeDate(when, +1) 
					}
				);
				const holidays = p.dv.pages(
				...
				p.info("journal", holidays.length, holidays);p.info("journal", holidays.length, holidays, holiday);

				let events = Array.from(p.dv.pages([
