import {  createFnWithProps, kindError,  toPascalCase, stripParenthesis  } from "inferred-types"
...
const clientHandler = (p: KindModelPlugin) => <
	...
	async(): Promise<boolean | Error> => {
		...
		if (page) {
			...
			if (re.test(evt.source)) {
				...
				? stripParenthesis(Array.from(
					evt.source.match(re) as RegExpMatchArray
				)[1])
				: "";
				...
				if (isError(result)) {
					...
					const handled = await handlerFn(event);
					p.debug(
						`Code Block event processed by ${handler}.`, {page,scalar,options}
					);
...
const addParams =  <THandler extends string>(
	handler: THandler
) => ({
	/**
	 ...
	 * ```
	 */
	scalar: <S extends readonly ScalarDefn[]>(
		...scalarParams: S
	) => ({
		/**
		...
		options: <O extends OptionsDefn>(optionParams: O = {} as O) => ({
			...
			>(handlerFn: TEvent) => (p: KindModelPlugin) =>  {
					return  (evt: ObsidianCodeblockEvent) => clientHandler(p)(
						handler,
						handlerFn,
						scalarParams,
						optionParams,
...
// 	GlobalCodeBlockHandler<
// 		THandler, 
// 		TParams
// 	> => (p: KindModelPlugin) => (
// 		source: string,
// 		el: HTMLElement,
// 		ctx: MarkdownPostProcessor & Component
// 	) => async (
// 		event: Handler<
// 			ScalarParams<S>,
// 			FromHandlerOptions<O>
// 		>
// 	): Promise<void> => {
// 		//
// 	}
