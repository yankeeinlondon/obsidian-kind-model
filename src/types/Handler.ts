...
import { KindError, KindErrorDefn } from "@yankeeinlondon/kind-error";
...
> {
	...
	/** log an error to dev console  */
	error: KindModelPlugin["error"];

	/**
	 * A hook to report an error within the handler
	 */
	report: KindErrorDefn<
		string, 
		{ evt: ObsidianCodeblockEvent, page: PageInfoBlock | undefined }
	>;

	/**
	 * a handler can express area they are working on to give the error\
	 * report more context.
	 */
	// workingOn: (area: string) => void;
