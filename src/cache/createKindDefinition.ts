...
import { getKindTagsOfPage } from "~/api/buildingBlocks";
	...
	if (pg) {
		try {
			...
			const defn: Omit<KindDefinition<["path"]>, "hash"> = {
				...
				metricProps: (
					pg._metricProps || pg.__metricProps
				) as Metric[]
			...
			p.error(`Problem calling createKindDefinition(${String(ref)}): ${msg}`);
		}
	} else {
		if (!ref) {
			p.warn(`an empty reference was passed to createKindDefinition!`);
		} else {
