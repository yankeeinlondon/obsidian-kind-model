...
export const lookupKindByTag = (p: KindModelPlugin) => (tag: string) => {
	...
	return p.cache.kindDefinitionsByTag.get(t) as KindDefinition<["tag", "path" | never]> | undefined;
}
...
export const lookupTypeByTag = (p: KindModelPlugin) => (tag: string) => {
	const t = stripLeading(tag, "#");
	return p.cache.typeDefinitionsByTag.get(t) as KindDefinition<["tag", "path" | never]> | undefined;
}
