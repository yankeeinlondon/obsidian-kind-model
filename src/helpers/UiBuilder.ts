...
import type {
  MaybeLazy,
  UiBuilderApi,
  UiBuilderComponentApi,
  UiBuilderContextApi,
  UiBuilderOptions,
} from "../types/ui-builder_types";
 ...
 */
function resolve<T extends MaybeLazy<any>>(val: T): string {
        ...
        addDropdown(choices) {
          s.addDropdown((dd) => {
            ...
            for (const opt of isKeyValueDict
              ...
              : (choices as string[])) {
              const value = isKeyValueDict
                ? String(choices[opt as keyof typeof choices])
                : opt;
            ...
            dd.onChange((v) => {
              if (isNotNull(prop, base)) {
                ...
                if (global_opt?.saveState && prop !== null) {
                  if (
                    typeof global_opt?.saveState !== "function"
                  ) {
                    error(
                      `saveState property was passed into UiBuilder but it's type is "${typeof global_opt?.saveState}" instead of being a function!`,
                    );
                  }
                  else {
                    info(
                  ...
                  debug(
                    `no auto save: state changed on "${name}" property but state is not automatically save after state changes`,
                  );
                }
                ...
                debug(
                  `the dropdown "${name}" changed state but no property was set to record this.`,
            ...
            t.onChange((v) => {
              if (isNotNull(prop, base)) {
                ...
                if (global_opt?.saveState && prop !== null) {
                  ...
                  debug(
                    `no auto save: state changed on "${name}" on property`,
                  );
                }
              }
          ...
          s.addText((t) => {
            ...
            t.onChange((v) => {
              if (isNotNull(prop, base)) {
                ...
                debug(
                  `state changed on the property "${name}" but because "prop" was null it will not be recorded.`,
                );
              }
              s.setName(resolve(name));
              s.setDesc(resolve(desc));
              ...
              if (isNotNull(prop, base)) {
                ...
                if (global_opt?.saveState) {
                  if (
                    ...
                    info(
                      `auto save`,
                      `folder prop ${name} [${prop}] changed state to:`,
                      v,
                    );
                    global_opt.saveState();
                  }
            ...
            b.setTooltip(o?.tooltip || resolve(desc))
              ...
              .onClick(
                o?.onClick
                  ...
                  : () =>
                      warn(
                        `${name} button for "${String(o?.buttonText)}" does not have a click handler`,
                      ),
              );
 ...
 */
function inputRow<TBase extends Record<string, any>, TGlobalOpt extends UiBuilderOptions>(el: HTMLElement, base: TBase, global_opt: TGlobalOpt, log_level: LogLevel) {
  return <TProp extends keyof TBase | null>(
    name: MaybeLazy<string>,
    desc: MaybeLazy<string>,
    prop: TProp,
    ...
    return componentApi(el, base, global_opt, log_level)(name, desc, prop)(
      s,
    );
  };
}
