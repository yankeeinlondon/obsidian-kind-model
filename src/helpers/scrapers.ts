 ...
 */
export const AmazonBook = async (p: KindModelPlugin, book: BookMeta): Promise<BookMeta> => {
	...
	try {
		...
		p.error(`Problems loading book information from Amazon (${url})`, e?.msg);
		return book;
	}
	...
	p.debug("Amazon", {
		...
		publisher, 
		publicationDate, 
		isbn10, isbn13, description,rating: amazonRating, reviewsAmazon
	})
