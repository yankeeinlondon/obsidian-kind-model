...
import type { DvPage, Link } from "~/types/dataview_types";
...
import type { PageBlock } from "~/types/settings_types";
 ...
 */
export function find_links<P extends DvPage>(links: ExternalLink[], fm_links: PropertyLink[], page: P) {
  return <T extends Traversable>(t: T) => {
    if (isObject(t) && isUrl(t?.attributes?.src)) {
      return [
        links.push({
          ...
          in_content: true,
          in_frontmatter: fm_links
            .map(i => i.url)
            .includes(t.attributes.src),
          tagName: t.name,
...
function linksInFrontmatter(fm: Frontmatter) {
 ...
 */
export function pageContent(p: KindModelPlugin) {
  return async <T extends TFile | Link | PagePath | DvPage>(
    ref: T,
  ): Promise<undefined | PageContent> => {
    ...
    const page = isDvPage(ref) ? ref : (p.dv.page(path) as DvPage);
    ...
    return {
      ...
      externalLinks: () => {
        ...
        const tree = renderableTree();
        const fm_links = linksInFrontmatter(page.file.frontmatter);
      ...
      internalLinks: () => {
        return isDvPage(ref)
          ...
          : (Array.from(
              (p.dv.page(path) as DvPage).file.outlinks,
            ) as Link[]);
      },
    };
  };
