...
export const showCreatedDate =
	(p: KindModelPlugin) =>
	(pg: PageReference | undefined, format?: string) => {
...
export const showModifiedDate =
	(p: KindModelPlugin) => (pg: PageReference, format?: string) => {
		const page = getPage(p)(pg);
		if (page) {
...
export const showWhen =
	(p: KindModelPlugin) =>
	(pg: PageReference | undefined, format: string = "LLL yyyy") => {
		const page = getPage(p)(pg);
...
export const showTags =
	(_p: KindModelPlugin) =>
	(pg: DvPage, ...exclude: string[]) => {
		return (
			pg.file.etags
				.filter(
					(t) =>
						!exclude.some((i) => (t.startsWith(i) ? true : false)),
				)
				.map((t) => `\`${t}\``)
				.join(", ") || ""
		);
...
export const getInternalLinks =
	(p: KindModelPlugin) =>
	(pg: PageReference | undefined, ...props: string[]) => {
		let links: FileLink[] = [];
		const page = getPage(p)(pg);
...
export const showLinks =
	(p: KindModelPlugin) => (pg: PageReference | undefined) => {
		const page = getPage(p)(pg);
		if (page) {
...
export const showProp =
	(p: KindModelPlugin) =>
	(pg: PageReference | undefined, ...props: [string, ...string[]]) => {
		const page = getPage(p)(pg);
...
export const showAbout =
	(_p: KindModelPlugin) =>
	(_pg: PageReference): FileLink[] => {
		return [] as FileLink[];
	};

export const showPeers =
	(_p: KindModelPlugin) =>
	(_pg: PageReference): string => {
		return "";
	};
/**
 * **getKind**
...
const link = (name: string, path: string, display: string) => {
	return [
...
export const htmlLink =
	(p: KindModelPlugin) =>
	(ref: PageReference | undefined, opt?: MarkdownLinkOpt): string => {
...
export const showCategories =
	(p: KindModelPlugin) =>
	(pg: PageReference, opt?: CategoryOptions): string => {
		const page = p.api.getPage(pg);
		let links: string[] = [];
...
export const showSubcategories =
	(p: KindModelPlugin) =>
	(pg: PageReference | undefined, opt?: CategoryOptions): string => {
		const page = p.api.getPage(pg);
		let links: string[] = [];
		// const withTag = isUndefined(opt?.withTag) ? true : opt.withTag;
...
export const showClassifications =
	(p: KindModelPlugin) =>
	(pg: PageReference): string => {
...
export const createMarkdownLink =
	(p: KindModelPlugin) =>
	(pathLike: PageReference | undefined, opt?: MarkdownLinkOpt): string => {
		const page = p.api.getPage(pathLike);
