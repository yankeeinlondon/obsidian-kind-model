...
const list_items_api = <W extends WrapperCallback>(
	wrapper: W,
): ListItemsApi<W> => ({
...
export const wrap_ol = (items: string, opts?: ListStyle) =>
	`<ol ${listStyle(opts)}>${items}</ol>`;

/** wrap text in `<ul>...</ul>` tags */
export const wrap_ul = (items: string, opts?: ListStyle) =>
	`<ul ${listStyle(opts)}>${items}</ul>`;
			...
			.map(
				(i) =>
					(isFunction(i)
						...
						: `<li ${style(opts?.li ? (isFunction(opts?.li) ? opts.li(i ? i : "") : opts.li) : {})}>${i}</li>`) as unknown as string,
			)
			.filter((i) => i !== "")
			.join("\n") as string,
		opts,
...
export const italic = (text: string | number, fmt?: Omit<StyleOptions, "fs">) =>
	italics(text, fmt);
